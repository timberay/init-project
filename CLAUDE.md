...
For new feature work, follow the six-phase pipeline. Skip only for bug fixes, refactors, or small tweaks. A `UserPromptSubmit` hook (`.claude/settings.json`) injects the phase reminder when a feature-request keyword is detected. **See**: [WORKFLOW.md](docs/standards/WORKFLOW.md).
