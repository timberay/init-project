...
# ADR-0001: quality gates moved to pre-commit/CI; Bash may still have safety guardrails.
...
! jq -e '.hooks.PreToolUse | map(.hooks[].command) | flatten | any(test("pytest|ruff|pre-commit"))' "$TMP/.claude/settings.json" >/dev/null \
